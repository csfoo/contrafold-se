                                                 ...
                                                 RealT hyperparam_data)
{
    ...
    return ComputeFunctionSE(units, w, toggle_use_nonsmooth, toggle_use_loss, log_base, hyperparam_data);   //added:param
                                                              ...
                                                              RealT hyperparam_data)
{
    ...
    return ComputeGradientSE(units, w, toggle_use_nonsmooth, toggle_use_loss, log_base, hyperparam_data);  // added:param
                                                    ...
                                                    const std::vector<RealT> &x)
{
    ...
    std::vector<RealT> g = ComputeGradient(units, x, false, true, GetOptions().GetRealValue("log_base"), GetOptions().GetRealValue("hyperparam_data"));
    RealT f = ComputeFunction(units, x, false, true, GetOptions().GetRealValue("log_base"), GetOptions().GetRealValue("hyperparam_data"));
    ...
    for (size_t k = 0; k < groups.size(); k++)
    {
        ...
        for (int i = groups[k].begin; num_left && i < groups[k].end; i++)
        {
            ...
            for (int j = 0; j < ATTEMPTS; j++)
            {
                ...
                xp[i] += EPSILON;
                gp[j] = (ComputeFunction(units, xp, false, true, GetOptions().GetRealValue("log_base"), GetOptions().GetRealValue("hyperparam_data")) - f) / EPSILON;
                xp[i] = x[i];
