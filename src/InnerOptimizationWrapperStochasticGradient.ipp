...
{
    return this->optimization_wrapper->GetComputationWrapper().ComputeFunctionSE(this->units, w, false, true, log_base, hyperparam_data) + RealT(0.5) * DotProduct(this->C, w*w) + DotProduct(w, this->bias);
}
...
void InnerOptimizationWrapperStochasticGradient<RealT>::ComputeGradient(std::vector<RealT> &g, const std::vector<RealT> &w, const int batch_size)
{
    ...
    g = this->optimization_wrapper->GetComputationWrapper().ComputeGradientSE(units, w, false, true, log_base, hyperparam_data) + this->C * w + this->bias;
}
