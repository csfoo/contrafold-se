...
{
    return this->optimization_wrapper->GetComputationWrapper().ComputeFunction(this->units, w, true, true, this->optimization_wrapper->GetOptions().GetRealValue("log_base"), this->optimization_wrapper->GetOptions().GetRealValue("hyperparam_data")) + RealT(0.5) * DotProduct(this->C, w*w) + DotProduct(w, this->bias);
}
...
{
    g = this->optimization_wrapper->GetComputationWrapper().ComputeGradient(this->units, w, true, true, this->optimization_wrapper->GetOptions().GetRealValue("log_base"), this->optimization_wrapper->GetOptions().GetRealValue("hyperparam_data")) + this->C * w + this->bias;
}
