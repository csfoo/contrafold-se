...
{
    return this->optimization_wrapper->GetComputationWrapper().ComputeFunction(this->units, w, false, true, log_base, hyperparam_data) + RealT(0.5) * DotProduct(this->C, (w - this->weights_initial)*(w - this->weights_initial)) + DotProduct(w, this->bias);
}
...
{
    g = this->optimization_wrapper->GetComputationWrapper().ComputeGradient(this->units, w, false, true, log_base, hyperparam_data) + this->C * (w - this->weights_initial) + this->bias;
}
...
void InnerOptimizationWrapperLBFGS<RealT>::Report(int iteration, const std::vector<RealT> &w, RealT f, RealT step_size)
{
    ...
    this->optimization_wrapper->PrintMessage(SPrintF("Inner iteration %d: f = %lf (%lf), |g| = %lf, |w| = %lf, step = %lf, efficiency = %lf%%", 
                                                     ...
                                                     double(Norm(this->optimization_wrapper->GetComputationWrapper().ComputeGradient(this->units, w, false, true, log_base, hyperparam_data) + this->C * (w-this->weights_initial) + this->bias)),
