...
{
    return this->optimization_wrapper->GetComputationWrapper().ComputeFunction(this->units, w, false, true, log_base, this->optimization_wrapper->GetOptions().GetRealValue("hyperparam_data")) + RealT(0.5) * DotProduct(this->C, w*w) + DotProduct(w, this->bias);
}
...
{
    g = this->optimization_wrapper->GetComputationWrapper().ComputeGradient(this->units, w, false, true, log_base, this->optimization_wrapper->GetOptions().GetRealValue("hyperparam_data")) + this->C * w + this->bias;
}
...
void InnerOptimizationWrapperEM<RealT>::Report(int iteration, const std::vector<RealT> &w, RealT f, const std::vector<RealT> &g, RealT step_size)
{
    ...
    this->optimization_wrapper->PrintMessage(SPrintF("Inner iteration %d: f = %lf (%lf) [%lf], |w| = %lf, |g| = %lf, step = %lf, efficiency = %lf%%", 
                                                     ...
                                                     double(this->optimization_wrapper->GetComputationWrapper().ComputeFunction(this->units, w, true, true, log_base, this->optimization_wrapper->GetOptions().GetRealValue("hyperparam_data")) + RealT(0.5) * DotProduct(this->C, w*w) + DotProduct(w, this->bias)),
