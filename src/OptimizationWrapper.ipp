        ...
        cached_C != C)
    {
        ...
        if (GetOptions().GetBoolValue("viterbi_parsing"))
        {
            ...
            for (int i = 0; i < NUM_CCCP_STEPS; i++)
            {
                ...
                if (i > 0) bias = -RealT(NONCONVEX_MULTIPLIER) * computation_wrapper.ComputeGradient(units, cached_learned_w, true, false, log_base, hyperparam_data);
                                                      ...
                                                      std::vector<RealT> &w)
{
    ...
    for (int k = -5; k <= 10; k++)
    {
        ...
        RealT loss = computation_wrapper.ComputeFunction(holdout, x, false, false, log_base, hyperparam_data);
#else
